...
*/
	...
	while {true} do
	{
		...
		PMC_Weather_forecast = format["Weather report: %1, change time %2 min, duration %3 min.", PMC_do_overcast, (floor (PMC_WeatherChangeTime / 60)), (floor (_duration / 60))];
...
*/
		...
		// debug :)
		diag_log format["PMC_weather.sqf Sleep PMC_WeatherChangeTime: %1 min, _duration: %2 min, PMC_do_overcast: %3", (PMC_WeatherChangeTime / 60), (_duration / 60), PMC_do_overcast];
