...
private ["_grp","_HeloGrp","_rtb","_landingzone","_vcl","_tmp"];
...
if (PMC_debug) then { diag_log format ["Infantry_Transport_Airborne(US) start with vehicle: %1, _grp: %2", _vcl, _HeloGrp]; };
...
if (PMC_debug) then { diag_log format ["Infantry Transport Airborne(US) %1, approach check. unitReady: %2, LZ dist: %3, canMove %4", _HeloGrp, unitReady _vcl, (_vcl distance _landingzone), canMove _vcl]; };
...
if (!alive _vcl || !alive leader _HeloGrp) exitWith
{
	...
	PMC_InfTransport = PMC_InfTransport - 1;
	publicVariable "PMC_InfTransport";
if (PMC_debug) then { diag_log format ["Infantry Transport Airborne(US) %1 has exited with catastrophic failure. alive _vcl: %2, alive leader _HeloGrp: %3, posit: %4", _HeloGrp, alive _vcl, alive leader _HeloGrp, getPosASL _vcl]; };
};
...
if (PMC_debug) then { diag_log format ["Infantry Transport Airborne(US) %1, RTB check. unitReady: %2, rtb dist: %3, canMove %4, _rtb: %5", _HeloGrp, unitReady _vcl, (_vcl distance _rtb), canMove _vcl, _rtb]; };

// if she arrived, if shes ok, then just delete her and her crew.
//if (alive _vcl && alive leader _HeloGrp) then
if (_vcl distance _rtb < 500) then
{
if (PMC_debug) then { diag_log format ["Infantry Transport Airborne(US)(%1) %2 has exited with (ALL OK) alive _vcl: %3, alive leader _HeloGrp: %4. POSIT: %5, _rtb: %6", PMC_InfTransport, _HeloGrp, alive _vcl, alive leader _HeloGrp, getPosASL _vcl, _rtb]; };
	{
		deleteVehicle _x;
	} forEach units _HeloGrp;
	deleteVehicle _vcl;
...
{
if (PMC_debug) then { diag_log format ["Infantry Transport Airborne(US)(%1) %2 has exited with (NOT OK) alive _vcl: %3, alive leader _HeloGrp: %4. POSIT: %5, _rtb: %6", PMC_InfTransport, _HeloGrp, alive _vcl, alive leader _HeloGrp, getPosASL _vcl, _rtb]; };
};
